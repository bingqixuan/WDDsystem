	...
	*/
	...
	public boolean onSingleTap(MotionEvent point) {
		...
		if(IDs.length>0)
		{
			/*
			//如果正在进行点位纠错
			if(IsCorrectingtPoint==true)
			{
				//将选中的图形的属性显示到点位纠错页面的listView里面
				Intent intent=new Intent();
				Graphic sGrapdic=layer.getGraphic(IDs[0]);
				
			    ArrayList<String> key=new ArrayList<String>();
				ArrayList<String> value=new ArrayList<String>();
				Map<String,Object> attribute=sGrapdic.getAttributes();
			    intent.putStringArrayListExtra("key",key);
			    intent.putStringArrayListExtra("value", value);
			    intent.setClass(context,PointCorrectionActivity.class);
